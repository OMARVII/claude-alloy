...
> **Status:** Submitted to the official Anthropic plugin marketplace — pending review. Once approved, install with a single command inside Claude Code.

```bash
# After approval:
/install claude-alloy

# Self-hosted marketplace (works now)
/plugin marketplace add OMARVII/claude-alloy
/plugin install claude-alloy@claude-alloy

# Local testing (clone first)
...
if ! command -v jq &>/dev/null; then
    echo '{"hookSpecificOutput":{"message":"[ALLOY] jq required."}}'
    exit 0
fi
