...
if command -v claude &>/dev/null; then
    if claude mcp add exa-websearch -s user -- npx -y exa-mcp-server@3.2.0 2>/dev/null; then
        info "Added websearch MCP server"
    fi
    if claude mcp add context7 -s user -- npx -y @upstash/context7-mcp@2.1.6 2>/dev/null; then
