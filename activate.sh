...
echo ""
info "MCP pinning (optional): claude-alloy ships ${BOLD}.mcp.json.example${NC} with"
info "  context7, grep_app, websearch pinned via alwaysLoad — prevents"
info "  schema-deferral bugs in subagents. Enable per-project with:"
info "    cp .mcp.json.example .mcp.json   (project scope)"
info "  or merge into ${BOLD}~/.claude.json${NC}       (user scope)"
info "  Full guide: ${BOLD}docs/mcp-config.md${NC}"
echo ""
