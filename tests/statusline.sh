...
OUT=$(make_json 0 | CLAUDE_PLUGIN_ROOT="$REPO_ROOT" bash "$HOOK" | strip_ansi)
assert_eq 1 "$(contains "$OUT" '1.6.4')" "version reads from CLAUDE_PLUGIN_ROOT/VERSION"
...
assert_eq 1 "$(contains "$OUT" '1.6.4')" "SCRIPT_DIR/../VERSION fallback when CLAUDE_PLUGIN_ROOT unset"
