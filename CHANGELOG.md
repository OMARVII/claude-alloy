...
> **Installer MCP-list reliability.** The `ensure_mcp` shortcut in `install.sh` and `activate.sh` (and the parallel scope filter in `doctor.sh`) was passing `--scope user` / `-s user` to `claude mcp list`, which has no scope flag — the command exited with `error: unknown option '--scope'`, stderr was swallowed by `2>/dev/null`, and the secondary grep pattern (`"\"$url\""`) assumed JSON output that the command does not produce. Net effect: the "already configured? skip" early-return never fired, so every install/activate run silently did a redundant `claude mcp remove` + `claude mcp add` for `context7`, `grep_app`, and `websearch`, slowing installs and wiping any user-customized auth headers on those server entries. The fix centralizes the call into a small `mcp_list()` helper (no flag, full-scope output), switches to `grep -Fq "$url"` for literal-URL matching against the plain-text output, and retains `--scope user` on the `mcp add` / `mcp remove` subcommands (which do accept it). No behavioral change to which servers get configured or in which scope — just makes the no-op path actually no-op.

### Fixed
- **`install.sh`, `activate.sh` — `ensure_mcp` early-return now actually fires.** `claude mcp list --scope user` is not a valid invocation (the `list` subcommand has no scope flag); the call silently failed, swallowed stderr via `2>/dev/null`, and returned empty stdout, so the `grep -q "\"$url\""` guard never matched. Both installers now use a local `mcp_list()` helper that calls `claude mcp list` with no flags and pipes through `grep -Fq "$url"` against the plain-text output. The redundant remove/re-add path for `context7`, `grep_app`, and `websearch` on every run is eliminated.
- **`doctor.sh` — same root cause, parallel fix.** The `check_mcp` healthcheck was using `claude mcp list -s user`, which is the same unknown option (short form). Dropped the flag; the loop that scans `MCP_LIST` for each required server still matches correctly because the plain-text output includes the server name token.
